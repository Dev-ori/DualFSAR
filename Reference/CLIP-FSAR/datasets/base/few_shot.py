...
class FewshotDataset(BaseVideoDataset):
    def __init__(self, cfg, split):
        super(FewshotDataset, self).__init__(cfg, split) 
        ...
        self.decode = self._decode_pickle
        
    def _decode_pickle(self, sample_info, index = None, num_clips_per_video=1):
        ...
        try : 
            with open(path, 'rb') as f:
                ...
                image_embeds = torch.from_numpy(frames['image_embeds'])
                q_former_image_embeds = torch.from_numpy(frames['q_former_image_embeds'])             # agnostic
                multimodal_embeds = torch.from_numpy(frames['multimodal_embeds'])   # specific
                
                q_features = torch.stack([q_former_image_embeds, multimodal_embeds])
        except:
            success = False
            
        return {"video" : image_embeds, 'q-fromer': q_features}, file_to_remove, success
    ...
    def get_seq(self, label, idx=-1):
        ...
        if self.cfg.TRAIN.META_BATCH:
            ...
            if self.dataset_name == 'Ssv2_few_shot':
                ...
                video_path = os.path.join(self.data_root_dir, paths.replace('.avi', '.pickle'))
            sample_info = {
                "path": video_path,
                # "supervised_label": class_,
            }
    ...
    def get_seq_query(self, label, idx=-1):
        ...
        if self.cfg.TRAIN.META_BATCH:
            ...
            if self.gpu_transform:
                for k, v in data.items():
                    data[k] = v.cuda(non_blocking=True)
                # data["multimodal_embeds"].cuda(non_blocking=True)
            # if self._pre_transformation_config_required:
            #     self._pre_transformation_config()
